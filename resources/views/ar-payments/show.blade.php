            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <table class="table table-borderless mb-0">
                        <tr><td class="text-muted" style="width:160px">Customer</td><td>{{ $payment->customer->name }}</td></tr>
                        ...
                        <tr><td class="text-muted">Belum Dialokasikan</td><td>{{ number_format($payment->unallocated_amount, 2) }}</td></tr>
                        <tr><td class="text-muted">Rekonsiliasi Bank</td><td>{{ $payment->reconciled ? 'Sudah' : 'Belum' }}</td></tr>
            ...
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr><th>No. Invoice</th><th>Dibayar</th><th>Diskon</th><th>Write-off</th><th>GL Write-off</th></tr>
                            </thead>
                            <tbody>
                                @foreach($payment->allocations as $allocation)
                                    <tr>
                                        <td><a href="{{ route('ar-invoices.show', $allocation->invoice) }}">{{ $allocation->invoice->invoice_no }}</a></td>
                                        ...
                                        <td>{{ $allocation->writeOffGlAccount?->name ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="card">
                <div class="card-body d-flex flex-column gap-2">
                    ...
                    @if($payment->status === 'draft')
                        @canWrite
                            <form method="POST" action="{{ route('ar-payments.submit', $payment) }}">
                                @csrf
                                <button type="submit" class="btn btn-primary w-100">Ajukan untuk Approval</button>
                            </form>
                            <form method="POST" action="{{ route('ar-payments.destroy', $payment) }}" onsubmit="return confirm('Hapus pembayaran ini?')">
                                @csrf @method('DELETE')
                    ...
                    @if($payment->status === 'diajukan')
                        @canApprove
                            <form method="POST" action="{{ route('ar-payments.approve', $payment) }}">
                                @csrf
                                <button type="submit" class="btn btn-success w-100">Setujui</button>
                            </form>
                            <form method="POST" action="{{ route('ar-payments.reject', $payment) }}">
                                @csrf
                    ...
                    @if($payment->status === 'disetujui' && ! $payment->reconciled)
                        @canWrite
                            <form method="POST" action="{{ route('ar-payments.mark-reconciled', $payment) }}">
                                @csrf
                    ...
                    <a href="{{ route('ar-payments.index') }}" class="btn btn-link">&larr; Kembali ke daftar</a>
                </div>
            </div>
        </div>
    </div>
